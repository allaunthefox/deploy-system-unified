...
Development packages are locked with `pip-tools` to preserve supply-chain verification:

1. Maintain direct requirements in `requirements-dev.in`.
2. Run:
   ```bash
   pip install pip-tools # in a venv or pipx
   pip-compile --generate-hashes requirements-dev.in -o requirements-dev.txt
   ```
3. Commit both files. CI uses:
   ```bash
   pip install --require-hashes -r requirements-dev.txt
   ```

(do **not** manually edit `requirements-dev.txt` as it is autogenerated.)
