...
This project handles virtualization networking through modular L2 and L3 isolation strategies. By separating these concerns into `core/virt_networking`, we ensure that physical network logic never comingles with virtual overlay logic.
...
2. **Encapsulation**:
   ...
   - **Mandatory Security**: Because VXLAN packets are plain L3 traffic, they **MUST** be paired with the `security/inter_pod_encryption` (IPSec/Wireguard) role if they traverse untrusted networks.
3. **Control Plane**:
...
To avoid comingling, the `core/virt_networking` role is divided into:
