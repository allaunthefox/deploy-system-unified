...
The layers work together via the **[Ontology](../architecture/ONTOLOGY.md)**. Selecting a `hardened` profile automatically coordinates the settings across all five layers, ensuring that a change in the firewall policy is reflected in the access control and scanning logic.
